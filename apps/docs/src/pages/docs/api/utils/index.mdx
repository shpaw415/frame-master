...
```ts
import { onRoute, join, pluginRegex } from "frame-master/utils";
...
Manage file directives across plugins. Import this from `frame-master/plugin/utils`, not `frame-master/utils` — the helper module stays free of plugin/server code so Worker and browser bundles can use `join` / `isProd` safely.
