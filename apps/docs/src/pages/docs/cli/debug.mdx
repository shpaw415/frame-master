...
global `plugin.build` merge) plus one selector entry per unifier bucket.
Each bucket inherits the same virtual-module registry as the default builder.

```ts
import { BuildUnifier, getBuildPipeline, getBuildUnifierContext } from "frame-master/plugin";
...
Bucket plugins register fragments with `getBuildUnifierContext()?.setBuildConfig`
in `createContext`, then await `getBuildUnifierContext("functions").getBuilder(name)`
or `getBuildPipeline("functions")`. Legacy `getGlobalPluginContext("build-unifier")`
still works. `buildPipeline()` remains as a deprecated alias.
