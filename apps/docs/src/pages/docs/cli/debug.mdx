...
- Virtual modules — plugin-declared `virtualModules` source content (resolved from a string, `Uint8Array`, or contents factory), the managed provider load, and every downstream transformation
...
`BuildUnifier()` is the core build-unifier: it groups plugins into an isolated
builder bucket. `frame-master debug build` always shows **Default build** (the
global `plugin.build` merge) plus one selector entry per unifier bucket, using
the wrap `label`. Each bucket inherits the same virtual-module registry as the
default builder.
...
Consumer plugins register fragments in `createContext` with
`getGlobalPluginContext("build-unifier")?.setBuildConfig(pluginName, fragment)`
(or `getBuildUnifierContext()?.setBuildConfig`). Trigger a dev build later from
`serverReady` / `serverStart.main` with `getBuildPipeline(pluginName).getBuilder(pluginName)`
and `builder.build()`. `getBuildPipeline(pluginName)` throws if that plugin is
not wrapped. See [Isolated build pipelines](/docs/core/build#isolated-build-pipelines)
for the full consumer lifecycle. `buildPipeline()` remains as a deprecated alias.
