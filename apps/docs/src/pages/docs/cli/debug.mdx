...
Wrap isolated, inspectable build pipelines with `buildPipeline()`.
Each pipeline has its own builder and appears in the debug UI's pipeline filter,
with its build history, files, and transforms preserved in exported traces.

```ts
import { buildPipeline, getBuildPipeline } from "frame-master/plugin";
...
Pipeline plugins can use `getBuildPipeline("functions")` to register a build
fragment or await the isolated builder. The existing
`getGlobalPluginContext("build-unifier")` `setBuildConfig` and `getBuilder`
methods remain available for legacy build-unifier consumers.
