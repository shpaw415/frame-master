...
```ts
import { BuildUnifier, getBuildPipeline, getGlobalPluginContext } from "frame-master/plugin";
...
Bucket plugins register fragments with `setBuildConfig` in `createContext`, then
await `getBuilder` (or `getBuildPipeline("functions")`). Legacy
`getGlobalPluginContext("build-unifier")` still works. `buildPipeline()` remains
