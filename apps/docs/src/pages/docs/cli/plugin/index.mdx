...
`bunfig.toml` preloads `test/preload.ts` before Bun evaluates test modules. The
preload registers runtime plugins declared by the plugin, so tests exercise the
same runtime plugin chain used in production. Keep this preload when adding tests
that import modules depending on runtime plugins.
