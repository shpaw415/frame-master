...
- `setBuildConfig(pluginName, fragment)` in `createContext` contributes
  `buildConfig` / `beforeBuild` / `afterBuild` to that bucket only.
- `getBuildPipeline("functions")` or `getGlobalPluginContext("build-unifier")?.getBuilder(name)`
  returns the isolated builder.
