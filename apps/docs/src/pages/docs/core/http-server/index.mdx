...
> `filePath` is the filename relative to the watched directory. `projectRootPath` is relative to the project root (cwd). `absolutePath` is the OS absolute path. Existing 3-argument callbacks remain valid.
