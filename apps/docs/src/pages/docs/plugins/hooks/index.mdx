...
- **Server Hooks**: `serverStart.main`, `serverStart.dev_main`, `serverReady`, `serverStop`, `createContext`
...
Runs after `Bun.serve` is listening. Receives `{ builder, pluginLoader, config, server }`. Runs again after a config reload creates a new server.

### serverStop

Inverse of start/ready. Runs on the **old** plugin generation before the loader is replaced, on `createPluginTestEnv().dispose()`, and on SIGINT/SIGTERM after `Serve()`.

Hooks run sequentially in **reverse priority** (highest number first). Errors are logged and do not block shutdown.

```ts
import { deleteGlobalPluginContext, getGlobalPluginContext } from "frame-master/plugin";

{
  name: "db",
  version: "1.0.0",
  createContext: async () => ({ pool: await openPool() }),
  serverStop: async ({ reason }) => {
    const ctx = getGlobalPluginContext("db");
    await ctx?.pool.close();
    deleteGlobalPluginContext("db");
  },
}
```

`reason` is `"reload" | "dispose" | "signal" | "explicit"`. `server` is the instance about to stop, or `null` when no HTTP server was started.

### createContext

Runs after the plugin loader and builder are ready. Return value is stored in `getGlobalPluginContext(plugin.name)`.
...
Respond to watched file changes (dev mode only). Callbacks may omit `absolutePath`; 3-argument plugins stay compatible.

```ts
onFileSystemChange: async (eventType, filePath, projectRootPath, absolutePath) => {
...
// Config reload
serverStop(old) → stop HTTP → reinit → onConfigReload(new) → Bun.serve → serverReady

// Ctrl+C / SIGTERM after Serve()
serverStop(reason: signal) → stop HTTP (force) → process.exit
