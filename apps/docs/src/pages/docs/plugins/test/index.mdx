...
// Import modules handled by the runtime plugins only after the preload.
import "../src/runtime-entry";
```

`loadRuntimePluginFromPlugins(plugins)` collects only the `runtimePlugins` declared by the provided `FrameMasterPlugin` instances and registers them with the same `onLoad` chaining behavior as the production runtime loader. It does not read `frame-master.config.ts`, create a `PluginLoader`, or start a `PluginTestEnv`.
