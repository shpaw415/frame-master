...
- `BuildUnifier()` pipelines are attached and initialized before `serverStart`. `dispose()` clears that registry so the next test can construct the same pipeline.
...
Runs Frame-Master’s **unified build** for plugins registered with a `build` section, with `BUILD_MODE` set for the call (same as `frame-master build`):

- Merges static and dynamic `buildConfig` from all plugins
- Runs `beforeBuild` / `afterBuild` hooks
- Builds every `BuildUnifier` pipeline, not only the default builder
