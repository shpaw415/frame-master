...
### Extendable Directive Interface

The directive system is now fully type-safe and extendable. Plugin authors can extend the `CustomDirectives` interface to add type-safe custom directives that work with both `DirectiveTool` and `FrameMasterPlugin.directives`.

**Extending the directive type system:**

```typescript
// In your plugin file, augment the module
...
// Now use in your plugin with full type safety
import type { FrameMasterPlugin } from "frame-master/plugin/types";
import { createDirective } from "frame-master/plugin/utils";

export default function analyticsPlugin(): FrameMasterPlugin {
  return {
    name: "analytics-plugin",
    version: "1.0.0",
    directives: [
      createDirective(
        "use-analytics", // Type-safe, autocomplete works!
        /^(?:\s*(?:\/\/.*?\n|\s)*)?['"]use[-\s]analytics['"];?\s*(?:\/\/.*)?(?:\r?\n|$)/m
      ),
    ],
  };
}
...
**New exports from `frame-master/plugin/utils`:**

- `DirectiveDefinition<T>` - Type for directive definitions with type-safe names
- `Directives` - Combined type of base and custom directives
- `BaseDirectives` - Built-in directives: "use-client", "use-server", "use-static", "server-only"
- `CustomDirectives` - Extendable interface for custom directives
- `createDirective(name, regex)` - Helper to create type-safe directive definitions
- `createCustomDirective(name, regex)` - Helper for arbitrary directive names (not type-checked)

**Example with DirectiveTool:**

```typescript
// After extending CustomDirectives
await directiveToolSingleton.pathIs("use-analytics", filePath); // ✓ Type-safe
await directiveToolSingleton.getPath("use-analytics"); // ✓ Autocomplete works
```
