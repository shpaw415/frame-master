...
export default function DebugApp() {
	...
	const deferredFileNameFilter = useDeferredValue(
	...
	return (
		<ThemeProvider theme={debugTheme} data-theme={theme}>
			<Box
				sx={{
					display: "flex",
					height: "100vh",
			...
			>
				...
				<AppBar position="static" elevation={0}>
					...
					>
						...
						>
							...
							<Chip size="small">
								watch {String(state.session?.options.watch ?? false)}
							</Chip>
							<Chip
								size="small"
								color={state.connection === "open" ? "success" : "warning"}
							>
								{state.connection}
							</Chip>
						</Box>
						...
						>
							<Button
								...
								title={
									isLive
										? "Trigger a new build"
										: "Unavailable while viewing a saved trace"
								}
							>
								Rebuild
							</Button>
				...
				<Box
					sx={{ display: "flex", minHeight: 0, flex: 1, overflow: "hidden" }}
				>
					<Box
						sx={{
											...
											<DebugFilterSelect
												...
												onChange={(pipelineId) => {
													...
													if (nextBuild) {
														startTransition(() => {
															setState((current) =>
																selectBuild(current, nextBuild.id),
															);
														});
													}
												}}
													...
													return (
														...
														>
															...
															<Typography
																...
																color="textSecondary"
															>
																{build.fileCount} files · {build.stepCount}{" "}
																steps · {build.outputCount} outputs
															</Typography>
														</ListItemButton>
													);
												})}
									...
									>
										<Box
											sx={{
												...
												gridTemplateColumns: {
													xs: "minmax(0, 1fr)",
													sm: "minmax(0, 1fr) minmax(132px, 0.4fr)",
												},
												gap: 1.5,
												p: 1.5,
											}}
													...
													return (
														...
														>
															...
															<Typography
																...
																variant="caption"
															>
																{step.sizeBefore} bytes → {step.sizeAfter} bytes
															</Typography>
															{step.error && (
																<Typography
																	color="error"
											...
											{deferredRegistry.length === 0 ? (
												...
												deferredRegistry.map((plugin) => (
													...
													>
														...
														<Typography color="textSecondary" variant="caption">
															{plugin.metrics.fileCount} files ·{" "}
															{plugin.metrics.buildCount} builds ·{" "}
						...
						>
							{selectedSummary ? (
								<>
									...
									<Chip
										size="small"
										color={statusColor(selectedSummary.status)}
									>
									...
									<Typography color="textSecondary" variant="caption">
										{selectedSummary.fileCount} files
									</Typography>
									<Typography color="textSecondary" variant="caption">
										{selectedSummary.stepCount} steps
									</Typography>
								</>
							) : (
								<Typography color="textSecondary" variant="caption">
						...
						>
							...
							{selectedStep ? (
								<>
									...
									<Typography color="textSecondary" variant="caption">
										{selectedStep.sizeBefore} bytes → {selectedStep.sizeAfter}{" "}
										bytes
									</Typography>
									{selectedFile && (
										<Typography
											color="textSecondary"
											noWrap
											sx={{ maxWidth: { xs: "100%", sm: 320 }, minWidth: 0 }}
											variant="caption"
										>
											{selectedFile.path}
										</Typography>
