...
Frame-Master extends Bun's native types - **no imports needed**! `args.pooled` and `preventChaining` are automatically available:

```typescript
// No type imports required - works out of the box!
...
#### `getPooledContents()` Helper

For convenience, use the helper to get chained or disk contents:

```typescript
import { getPooledContents } from "frame-master/plugin";

build.onLoad({ filter: /\.tsx$/ }, async (args) => {
  // Automatically gets chained contents or reads from disk
  const { contents, loader } = await getPooledContents(args);
  return { contents: transform(contents), loader: loader ?? "tsx" };
});
```
