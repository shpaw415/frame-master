...
> = Required<{
	...
	Partial<{
		...
		/**
		 * Triggered after a server instance is created and ready to accept requests.
		 *
		 * Runs after all plugins have been initialized and after `serverStart` hooks
		 * have completed. This hook is awaited, so asynchronous setup is supported.
		 *
		 * This hook runs during the initial startup flow and again after config
		 * reloads that recreate the server.
		 **/
		serverReady: (props: {
			builder: Builder;
