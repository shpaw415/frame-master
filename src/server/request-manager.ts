...
import { webToken, type _webToken, type SetDataOptions } from "@shpaw415/webtoken";
...
export class MasterRequest<ContextType extends Record<string, unknown> = {}> {
    ...
    constructor(props: { request: Request }) {
        ...
        this.isAskingHTML = Boolean(this.request.headers.get("accept")?.includes("text/html"));
        /*
    ...
    private initMatch(): RequestMatch | undefined {
        if (this.isClientNavigating) {
            ...
            if (!pathname) throw new NoServerSideMatchError(`missing matching information for __BUNEXT_PATHNAME__`);
            ...
            if (!matchServer || !matchClient) throw new NoServerSideMatchError(`no matching route found for __BUNEXT_PATHNAME__`);
   ...
   */
    ...
    setResponse(body: BodyInit | null, init?: ResponseInit): this {
        ...
        if (this._response_setted) throw new ResponseAlreadySetError("Response already set");
   ...
   */
    ...
    public async toResponse(): Promise<Response> {
        if (this.__ERROR__) {
            const error = new FrameMasterError("Error occured during serving", this.__ERROR__)
            return this.setResponseThenReturn(new Response(
                renderToString(
                    errorToJSXPage(error, {}, "/frame-master-error.css"))
            )
            );
        }
    ...
    */
     ...
     */
    private async makePreLoadObject(): Promise<Partial<Record<keyof typeof globalThis, string>>> {
        try {
            return {
