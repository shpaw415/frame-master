...
function parseElement(
	html: string,
	startIndex: number,
): { element: ParsedElement; nextIndex: number } | null {
	const tagMatch = html
		.slice(startIndex)
		.match(/^<([a-zA-Z0-9-]+)([^>]*?)(\s*\/?)>/);
	...
	const isSelfClosing =
		selfClose?.trim() === "/" || VOID_ELEMENTS.has(tagName.toLowerCase());
...
function formatElement(
	...
	if (element.children && element.children.length > 0) {
		if (element.preserveWhitespace) {
			...
			const hasOnlyInlineChildren = element.children.every(
				(child) =>
					child.type === "text" || (child.type === "element" && child.isInline),
			);

			const shouldInline =
				options.preserveInlineElements &&
...
export function formatHTML(
	...
	const cleanedHtml = html.replace(/\s+/g, " ").replace(/>\s+</g, "><").trim();
